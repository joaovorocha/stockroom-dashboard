...
// POST /api/shipments/dedupe - Manually trigger deduplication
router.post('/dedupe', async (req, res) => {
  try {
    ...
    const beforeCount = loadShipments().length;
    const afterCount = dedupeShipmentsInPlace('manual-dedupe');
    const removed = beforeCount - afterCount;

    return res.json({
      success: true,
      message: `Deduplication complete. Removed ${removed} duplicate(s).`,
      before: beforeCount,
      after: afterCount,
      removed
    });
  } catch (error) {
    console.error('Error deduplicating shipments:', error);
    return res.status(500).json({ error: 'Failed to deduplicate shipments' });
  }
});
